...
class JwtEncoderTest extends TestCase
{
    ...
    public function test_encode_key_do_not_match()
    {
        ...
        $this->encoder->encode(
            ['foo' => 'bar'],
            new JWKSet([
                JWKFactory::createFromKeyFile($this->privateKey, null, ['use' => 'sig', 'alg' => 'RS128']),
            ])
        );
    }
    ...
    public function test_limit_supportedAlgorithms()
    {
        ...
        $this->assertNotEmpty($this->encoder->encode(['foo' => 'bar'], $jwks, 'RS256'));
        $this->assertNotEmpty($this->encoder->encode(['foo' => 'bar'], $jwks, 'HS256'));
        ...
        $this->assertNotEmpty($encoder->encode(['foo' => 'bar'], $jwks, 'RS256'));

        try {
            $encoder->encode(['foo' => 'bar'], $jwks, 'HS256');
